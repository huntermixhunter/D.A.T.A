#!/usr/bin/env bash
# DATA — macOS / Linux / ChromeOS uninstaller
# Reverses install.sh: stops DATA, removes the generated launcher, the app-menu
# entry, and (optionally) the bundled DATA-core skills. Leaves your data and the
# DATA folder itself in place — deleting the folder is the last step you do by hand.
set -e
...
echo ""
echo "  DATA — Uninstaller"
echo "  Removing what the installer added outside the DATA folder."
echo ""
...
# 1. Stop DATA (the bridge listens on 7777; the start script execs it directly).
echo "  [..] Stopping DATA..."
...
if [ -f "$DESKTOP_ENTRY" ]; then
    rm -f "$DESKTOP_ENTRY"
    echo "  [OK] Removed app-menu launcher (DATA)"
else
    echo "  [--] No app-menu launcher found (nothing to remove)."
fi

# 3. Remove the generated launcher.
if [ -f "$ROOT/start_data.sh" ]; then
    rm -f "$ROOT/start_data.sh"
    echo "  [OK] Removed start_data.sh"
fi
...
# 5. Bundled DATA-core skills — shared with the rest of your Claude setup, so
#    KEPT by default. Only unmodified copies DATA placed are removed, opt-in.
if [ -f "$ROOT/dashboard/uninstall_skills.py" ]; then
    if [ "$REMOVE_SKILLS" = "1" ] || confirm "Also remove the bundled DATA-core skills it installed?" 0; then
        PY=""
...
echo "    - Your AI provider CLI (claude / codex / ...) — DATA did not install it."

# 7. Final step: the folder. The script runs from inside it, so the user deletes it.
echo ""
echo "  Done. DATA is uninstalled."
