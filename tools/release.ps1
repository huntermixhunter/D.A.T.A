# DAITA - buyer-zip release builder
# Usage:  .\tools\release.ps1 1.0.0
# Produces dist\DAITA-v<version>.zip from tracked files only (git archive),
...
$zip = "$root\dist\DAITA-v$Version.zip"
...
git archive --format=zip --prefix="DAITA/" -o $zip HEAD
...
if (-not (git tag -l "v$Version")) {
    git tag -a "v$Version" -m "DAITA v$Version"
...
Write-Host "  Buyer instructions: unzip, then run install\install.bat (Windows)"
