...
- **Provider dropdown** — pick which AI backend runs (Claude / Codex / Gemini / Ollama). Detected automatically at startup.
...
- **Providers** are swappable per conversation from the dropdown. Different panes can
  even run different providers side by side — useful for getting a second model's
