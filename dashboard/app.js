...
function showPanel(name) {
  ...
  const btnMap = { chat: 0, matrix: 1, orders: 2, news: 3, weather: 3 };
...
// ═══════════════════════════════════════════════════════════
// NEWS FEED — news/YouTube aggregator
// ═══════════════════════════════════════════════════════════
const NEWS_SECTIONS = ['markets', 'crypto', 'spiritual', 'youtube', 'instagram', 'pinterest'];

async function loadNews() {
  _setupBriefingScrollHide();
  try {
    const res  = await fetch(`${API_BASE}/news`);
    const data = await res.json();
    _renderNews(data);
  } catch (e) {
    addLog(`News load failed: ${e.message || e}`);
  }
}

// Hide the News Feed header (title bar + filter chips) when the
// user scrolls down through the feed on mobile, and bring it back when
// they scroll up. Listener attached once; subsequent calls are no-ops.
let _briefingScrollWired = false;
function _setupBriefingScrollHide() {
  if (_briefingScrollWired) return;
  const mix = document.getElementById('news-mix');
  const panel = document.getElementById('panel-news');
  if (!mix || !panel) return;
  _briefingScrollWired = true;
  let lastY = 0;
  let ticking = false;
  mix.addEventListener('scroll', () => {
    if (ticking) return;
    ticking = true;
    requestAnimationFrame(() => {
      const y = mix.scrollTop;
      const delta = y - lastY;
      // Hysteresis: ignore jitter so small fingertip wiggles don't flicker.
      if (Math.abs(delta) > 8) {
        if (delta > 0 && y > 40) panel.classList.add('briefing-collapsed');
        else if (delta < 0)      panel.classList.remove('briefing-collapsed');
        lastY = y;
      }
      ticking = false;
    });
...
async function refreshNews() {
  ...
  try {
    ...
    const data = await res.json();
    if (data.error) {
      addLog(`Refresh failed: ${data.error}`);
    } else {
      _renderNews(data);
      addLog('News refreshed');
    }
  } catch (e) {
    addLog(`Refresh error: ${e.message || e}`);
  } finally {
    if (btn) { btn.disabled = false; btn.textContent = 'REFRESH'; }
  }
}

// ── Local weather (NWS forecast for Sandpoint, ID) ─────────
let _weatherLoaded = false;
async function loadWeather(force) {
  const fc = document.getElementById('weather-forecast');
  if (!force && _weatherLoaded) return;
  if (fc && !_weatherLoaded) fc.innerHTML = '<div class="weather-empty">Loading forecast…</div>';
  try {
    const url = `${API_BASE}/weather${force ? '?refresh=1' : ''}`;
    const res = await fetch(url);
    const data = await res.json();
    if (data.error) { if (fc) fc.innerHTML = `<div class="weather-empty">Weather offline — ${data.error}</div>`; return; }
    _renderWeather(data);
    _weatherLoaded = true;
  } catch (e) {
    if (fc) fc.innerHTML = `<div class="weather-empty">Bridge offline — cannot load weather</div>`;
    addLog(`Weather load failed: ${e.message || e}`);
  }
}
...
function _renderWeather(d) {
  ...
  if (locEl && loc.city) locEl.textContent = `${loc.city}, ${loc.state}`.toUpperCase();
  const srcEl = document.getElementById('weather-source');
  if (srcEl && d.source_url) srcEl.href = d.source_url;
  const tsEl = document.getElementById('weather-timestamp');
  if (tsEl) {
    const t = d.updated ? new Date(d.updated) : new Date();
    tsEl.textContent = 'UPDATED ' + t.toLocaleString([], { month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit' });
  }
  ...
  if (curEl) {
    if (c && c.temp_f != null) {
      const bits = [];
      if (c.wind_mph != null) bits.push(`Wind ${c.wind_mph} mph${c.gust_mph ? ' (gust ' + c.gust_mph + ')' : ''}`);
      if (c.humidity != null) bits.push(`Humidity ${c.humidity}%`);
      curEl.innerHTML = `
        <div class="wx-cur-icon">${c.icon ? `<img src="${c.icon}" alt="">` : ''}</div>
        <div class="wx-cur-main">
          <div class="wx-cur-temp">${c.temp_f}&deg;</div>
          <div class="wx-cur-text">${c.text || ''}</div>
          <div class="wx-cur-meta">${bits.join(' · ')}</div>
        </div>
        <div class="wx-cur-label">NOW</div>`;
      curEl.style.display = '';
    } else {
  ...
  // Forecast periods (day/night, ~7 days)
  const fc = document.getElementById('weather-forecast');
  if (!fc) return;
  const periods = d.periods || [];
  if (!periods.length) { fc.innerHTML = '<div class="weather-empty">No forecast data.</div>'; return; }
  fc.innerHTML = periods.map(p => {
    const pop = (p.probabilityOfPrecipitation || {}).value;
    const popHtml = (pop != null && pop > 0) ? `<span class="wx-pop">💧 ${pop}%</span>` : '';
    return `
      <div class="wx-period ${p.isDaytime ? 'day' : 'night'}">
        <div class="wx-period-head">
          <span class="wx-period-name">${p.name}</span>
          <span class="wx-period-temp">${p.temperature}&deg;${p.temperatureUnit}</span>
        </div>
        <div class="wx-period-body">
          ${p.icon ? `<img class="wx-period-icon" src="${p.icon}" alt="">` : ''}
          <div class="wx-period-text">
            <div class="wx-period-short">${p.shortForecast} ${popHtml}</div>
            <div class="wx-period-wind">${p.windDirection || ''} ${p.windSpeed || ''}</div>
            <div class="wx-period-detail">${p.detailedForecast || ''}</div>
          </div>
        </div>
      </div>`;
  }).join('');
...
// Active filter for the mixed feed. 'all' = everything; otherwise restrict.
let _newsFilter = 'all';
// Cached most-recent payload so filter clicks don't need a re-fetch.
let _newsPayload = null;
...
function _renderNews(data) {
  ...
  // Filter chips: ALL | WORLD | CRYPTO | SPIRITUAL | YOUTUBE
  const chipRow = document.getElementById('news-filter-row');
  if (chipRow && !chipRow.children.length) {
    const chips = [
      ['all', '✦ ALL'], ['markets', '$ MARKETS'], ['crypto', '₿ CRYPTO'],
      ['spiritual', '🕉 SPIRITUAL'], ['youtube', '▶ YOUTUBE'],
      ['instagram', '◉ INSTAGRAM'], ['pinterest', '📌 PINTEREST'],
    ];
    chipRow.innerHTML = chips.map(([k, label]) => `
      <button class="news-chip${k === _newsFilter ? ' active' : ''}" data-section="${k}"
              onclick="setNewsFilter('${k}')">${label}</button>
    `).join('');
  }
...
function setNewsFilter(section) {
  _newsFilter = section;
  document.querySelectorAll('.news-chip').forEach(c => {
    c.classList.toggle('active', c.dataset.section === section);
  });
  _renderMixedFeed();
}

function _renderMixedFeed() {
  const mix  = document.getElementById('news-mix');
  if (!mix) return;
  const sections = _newsPayload?.sections || {};
  // Flatten all sections into one list, tagging each item with its section
  let items = [];
  for (const sec of NEWS_SECTIONS) {
    for (const it of (sections[sec] || [])) {
      if (_newsFilter !== 'all' && _newsFilter !== sec) continue;
      items.push({ ...it, section: sec });
    }
  }
  if (!items.length) {
    mix.innerHTML = `<div class="news-empty">No items. Add sources via the SOURCES button.</div>`;
    return;
  }
  // Deterministic shuffle so the order is stable within a page-load
  items = _seededShuffle(items, _newsPayload?.updated_at || 0);

  // Assign tile sizes for varied masonry layout. Deterministic per-item so
  // hover/filter doesn't reshuffle visual sizes.
  //   YouTube → mostly bigger (size-l span-2-cols, occasionally size-xl)
  //   Articles with thumb → mix of size-s (most), size-m (some), size-l (few)
  //                         every ~10th gets .tile-feature (hero overlay)
  //   Articles without thumb → size-s or size-m, .tile-text styling
  let featureCounter = 0;
  let bigCounter = 0;
  mix.innerHTML = items.map((it, i) => {
    let cls = 'tile';
    // Hash from item link → stable per-item pseudo-random in [0, 1)
    const h = _strHash(it.link || it.title || '');
    const r = ((h % 1000) / 1000);

    if (it.section === 'youtube') {
      cls += ' tile-video';
      // 1 in 4 videos goes XL (huge hero), rest are L (2-col-wide standard)
      cls += (r < 0.25) ? ' size-xl' : ' size-l';
    } else if (it.thumbnail) {
      // Article with image
      featureCounter++;
      if (featureCounter % 10 === 0) {
        cls += ' tile-feature size-l';
      } else if (r < 0.15 && bigCounter < items.length / 8) {
        cls += ' size-l';     // occasional 2-col-wide standard
        bigCounter++;
      } else if (r < 0.40) {
        cls += ' size-m';     // taller card with bigger thumb
      } else {
        cls += ' size-s';     // default compact card
      }
    } else {
      cls += ' tile-text';
      cls += (r < 0.30) ? ' size-m' : ' size-s';
    }
    // YouTube thumbnail fallback chain: maxresdefault → hqdefault → sddefault.
    // Some videos don't have maxresdefault generated; the onerror cycles down.
    const thumbErr = it.thumbnail && it.thumbnail.includes('ytimg.com')
      ? `onerror=\"if(this.src.includes('maxresdefault')){this.src=this.src.replace('maxresdefault','hqdefault');return;}if(this.src.includes('hqdefault')){this.src=this.src.replace('hqdefault','sddefault');return;}this.parentNode.style.display='none';\"`
      : `onerror=\"this.parentNode.style.display='none';\"`;
    return `
      <div class="${cls}" data-news-idx="${i}" data-section="${it.section}" role="link" tabindex="0">
        ${it.thumbnail ? `<div class="tile-thumb-wrap"><img class="tile-thumb" src="${escapeHtml(it.thumbnail)}" loading="lazy" referrerpolicy="no-referrer" ${thumbErr}/><div class="tile-thumb-badge tile-badge-${it.section}">${_sectionLabel(it.section)}</div></div>` : ''}
        <div class="tile-body">
          ${!it.thumbnail ? `<div class="tile-section-badge tile-badge-${it.section}">${_sectionLabel(it.section)}</div>` : ''}
          <div class="tile-title">${escapeHtml(it.title)}</div>
          ${cls.includes('tile-text') && it.summary ? `<div class="tile-summary">${escapeHtml(it.summary)}</div>` : ''}
          <div class="tile-meta">${escapeHtml(it.source)} · ${it.ts ? _relTime(it.ts) : ''}</div>
        </div>
      </div>
    `;
  }).join('');

  // Stash the resolved items so click handlers can fetch by index without
  // re-running the shuffle + size assignment.
  _newsItemsForReader = items;

  // Wire single-click → inline modal, double-click → external site.
  // 240ms is the smallest gap that still feels intentional for a double-tap.
  mix.querySelectorAll('.tile[data-news-idx]').forEach(tile => {
    let timer = null;
    const openExternal = () => {
      const idx = +tile.dataset.newsIdx;
      const it = _newsItemsForReader[idx];
    ...
    tile.addEventListener('click', (e) => {
      ...
      if (timer) {
        ...
        timer = setTimeout(() => {
          timer = null;
          const idx = +tile.dataset.newsIdx;
          openNewsReader(_newsItemsForReader[idx]);
        }, 240);
      }
    });
...
// ── Inline reader for the News Feed ────────────────────
// Single-click a tile → modal opens here. YouTube items embed the
// player; articles get a rich preview (most sites block iframes via
// X-Frame-Options so embedding the live article isn't reliable).
let _newsItemsForReader = [];
...
function openNewsReader(item) {
  ...
  const sectionLabel = _sectionLabel(item.section);
  const when = item.ts ? _relTime(item.ts) : '';
  const escTitle  = escapeHtml(item.title || '');
  const escSource = escapeHtml(item.source || '');
  const escLink   = escapeHtml(item.link || '');

  let html = '';
  const ytId = item.section === 'youtube' ? _youtubeId(item.link) : null;
  if (ytId) {
    // YouTube — embed the player. ?autoplay=1 plus the muted-fallback so
    // browsers that block autoplay-with-sound still start playing.
    html = `
      <div class="news-reader-header">
        <span class="news-reader-badge tile-badge-youtube">${sectionLabel}</span>
        <span class="news-reader-source">${escSource}</span>
        ${when ? `<span class="news-reader-time">${when}</span>` : ''}
      </div>
      <h2 class="news-reader-title">${escTitle}</h2>
      <div class="news-reader-video">
      ...
      <div class="news-reader-actions">
        <a class="news-reader-external" href="${escLink}" target="_blank" rel="noopener noreferrer">↗ Open on YouTube</a>
      </div>`;
  } else {
    // Article — rich preview with summary. No iframe (most publishers
    // block embedding); the captain opens the original via the button.
    const escSummary = escapeHtml(item.summary || '');
    html = `
      <div class="news-reader-header">
        <span class="news-reader-badge tile-badge-${item.section}">${sectionLabel}</span>
        <span class="news-reader-source">${escSource}</span>
        ${when ? `<span class="news-reader-time">${when}</span>` : ''}
      </div>
      <h2 class="news-reader-title">${escTitle}</h2>
      ${item.thumbnail ? `<img class="news-reader-image" src="${escapeHtml(item.thumbnail)}" referrerpolicy="no-referrer" loading="lazy"/>` : ''}
      ${escSummary ? `<p class="news-reader-summary">${escSummary}</p>` : '<p class="news-reader-summary news-reader-empty">(No summary supplied by the feed.)</p>'}
      <div class="news-reader-actions">
        <a class="news-reader-external" href="${escLink}" target="_blank" rel="noopener noreferrer">↗ Open Original Article</a>
      </div>`;
  }

  body.innerHTML = html;
  modal.hidden = false;
  document.addEventListener('keydown', _newsReaderEscHandler);
}

function _newsReaderEscHandler(e) {
  if (e.key === 'Escape') closeNewsReader();
}

function closeNewsReader(e) {
  // If invoked from the backdrop click, ignore card clicks (event.stopPropagation in HTML).
  if (e && e.target && e.target.id !== 'news-reader-modal' && e.type === 'click') return;
  const modal = document.getElementById('news-reader-modal');
  const body  = document.getElementById('news-reader-body');
  if (modal) modal.hidden = true;
  // Wipe the body so a YouTube iframe stops playing audio immediately.
  if (body)  body.innerHTML = '';
  document.removeEventListener('keydown', _newsReaderEscHandler);
}

function _sectionLabel(sec) {
  return ({ markets: '$ MARKETS', crypto: '₿ CRYPTO', spiritual: '🕉 SPIRITUAL', youtube: '▶ YOUTUBE', instagram: '◉ INSTAGRAM', pinterest: '📌 PINTEREST' })[sec] || sec;
}

// Tiny string hash for deterministic per-item pseudo-random size assignment
function _strHash(s) {
  let h = 0;
...
// Stable seeded shuffle so the same payload always produces the same layout
// (no jumping items on filter toggle), but different refreshes reshuffle.
function _seededShuffle(arr, seed) {
  let s = (Math.floor(seed) || 1) & 0x7fffffff;
  const out = arr.slice();
...
async function openNewsSources() {
  try {
    const res  = await fetch(`${API_BASE}/news/sources`);
    const data = await res.json();
    const s = data.sources || {};
    for (const sec of NEWS_SECTIONS) {
      const ta = document.getElementById(`news-src-${sec}`);
      if (ta) ta.value = (s[sec] || []).join('\n');
    }
    document.getElementById('news-sources-dialog').classList.remove('hidden');
  } catch (e) {
    addLog(`Sources load failed: ${e.message || e}`);
  }
}

function closeNewsSources() {
  document.getElementById('news-sources-dialog').classList.add('hidden');
}
...
async function saveNewsSources() {
  ...
  for (const sec of NEWS_SECTIONS) {
    const ta = document.getElementById(`news-src-${sec}`);
    payload.sources[sec] = (ta?.value || '').split('\n').map(s => s.trim()).filter(Boolean);
  }
  try {
    const res  = await fetch(`${API_BASE}/news/sources`, {
      method:  'POST',
    ...
    const data = await res.json();
    if (data.error) { addLog(`Sources save failed: ${data.error}`); return; }
    addLog('Sources saved — refreshing…');
    closeNewsSources();
    refreshNews();
  } catch (e) {
    addLog(`Sources save error: ${e.message || e}`);
  }
}
